...
class day56p3{
        public static void main(String args[]){
		  ...
		  int[] ItemH = new int[nitems];
		  int[] RoomH = new int[nrooms];
		  int[] UsableRH = new int[nrooms];
		  
		  for(int i=0;i<nitems;i++) ItemH[i] = sc.nextInt();
		  for(int i=0;i<nrooms;i++) RoomH[i] = sc.nextInt();
		  
		  Arrays.sort(ItemH);
		 
		  for(int i=0;i<nrooms;i++){
		      int leftmin=RoomH[i];
		      int rightmin=RoomH[i];
		      for(int j=0;j<i;j++) leftmin = Math.min(leftmin,RoomH[j]);
		      
		      for(int j=i;j<nrooms;j++) rightmin = Math.min(rightmin,RoomH[j]);
		      
		      UsableRH[i] = Math.max(leftmin,rightmin);
		  }
		  
		  Arrays.sort(UsableRH);
		  
		  int i=nitems-1,j=nrooms-1,ans=0;
		  while(i>=0 && j>=0){
		      if(ItemH[i]<=UsableRH[j]){
		          j--;
		          ans++;
		      }
		      i--;
