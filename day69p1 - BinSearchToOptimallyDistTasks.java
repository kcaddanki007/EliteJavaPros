...
class Solution{
    public static void main(String[] args){
        ...
        for(int i=0;i<n;i++) workload[i] = sc.nextInt();
        System.out.println(getMin(n,p,workload));
    }
    static int getMin(int n,int m,int[] workload){
		    int max=0;
		    for(int i=0;i<n;i++){
		        max = Math.max(workload[i],max);
		    }
		    int low = 1, high = max ,ans=-1;
            while(low<=high){
                int mid= low +(high-low)/2;
                if(isPossible(n,mid,m,workload)){
                    ans = mid;
                    high = mid-1;
                }else 
                  low = mid+1;
		...
		static boolean isPossible(int n,int size,int p,int[] workload){
		    int count=0;
		    //System.out.println("mid "+size);
		    for(int i=0;i<n;i++){
		        count += workload[i]/size;
		        if (workload[i]%size > 0) ++count; 
		    }
		    //System.out.println("count "+count);
		    if(count<=n+p) return true;
		    else return false;
		}
}     
