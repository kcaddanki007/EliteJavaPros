...
class day18p2{
        public static void main(String args[]){
		  ...
		  for(int i=1;i<nrooms;i++){
		      if(RoomH[i]>UsableRH[i-1]) UsableRH[i] = UsableRH[i-1];
		      else UsableRH[i] = RoomH[i];
		  }
		  ...
		  int j=0;
		  while(i>-1 && j<nitems){
		      if(UsableRH[i] >= ItemH[j]){
		          count++;
		          i--;
		          j++;
		      }else if(UsableRH[i] < ItemH[j]) i--;
		  }
		  System.out.println(count);
		  //System.out.println(Arrays.toString(UsableRH));
		}
}        
