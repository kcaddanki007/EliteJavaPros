...
class Solution{
    public static void main(String[] args){
        ...
        System.out.println(getPossible(nums,n,k,0,dp));
    }
    static int getPossible(int[] nums,int n,int k,int curr,int[] dp){
        if(curr>=n) return 0;
        if(dp[curr]!=-1) return dp[curr];
        
        int ans = 0, ele=0, block=0;
        for(int i=curr;i<n && block<k;i++){
            block++;
            ele = Math.max(ele,nums[i]);
            ans = Math.max(ans,ele*block+getPossible(nums,n,k,i+1,dp));
        }
        return dp[curr]=ans;
    }
}