...
-65 -85 60 -30 40 -35
10 35 65 80 105 125
45 75 85 100 110 120

What is essentially happening is, the bigger the negative difference, the lesser the a[i][0] value
compared to the a[i][1] value. So for the first half we pick that. Then as the difference is not very
high, we can go on and pick a[i][1]
...
class Solution{
    ...
    static int getMinCost(int[][] arr,int n){
        Arrays.sort(arr,(p,q)->Integer.compare(p[0]-p[1],q[0]-q[1]));
        int ans=0;
        for(int i=0;i<n;i++){
            if(i<n/2) ans+=arr[i][0];
            else ans+=arr[i][1];
        }
        return ans;
    }
