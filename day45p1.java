...
class day45p1{
        public static void main(String args[]){
		  ...
		  System.out.println(generateSubset(arr,n));
		}
		static boolean generateSubset(int[] arr,int n){
		    if(n<7) return false;
		    int sum=0;
		    for(int i=0;i<n-6;i++){
		        sum+=arr[i];
		        if(isPossible(arr,sum,i+2,n)) return true;
		    }
		    return false;
		}
		static boolean isPossible(int[] arr,int sum,int i,int n){
		    int tempsum=sum;
		    int count=0;
		    for(int j=i;j<n;j++){
		        if(tempsum-arr[j]<0) return false;
		        else if(tempsum-arr[j]>0){
		            tempsum-=arr[j];
		            continue;
		        }
		        else if(tempsum-arr[j]==0){
