...
class Solution{
    public static void main(String[] args){
        ...
        System.out.println(getCountOfUnsavedPeopleDFS(forest,n));
    }
    static int getCountOfUnsavedPeopleDFS(int[][] arr,int n){
        for(int i=0;i<n;i++){
            DFS(arr,i,0,n);
            DFS(arr,0,i,n);
            DFS(arr,i,n-1,n);
        ...
        for(int i=0;i<n;i++) 
          for(int j=0;j<n;j++)
            if(arr[i][j]==1) countRem++;
        
        return countRem;
    }
    static void DFS(int[][] arr,int i,int j,int n){
