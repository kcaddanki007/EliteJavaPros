...
class Solution{
    public static void main(String[] args){
        ...
        System.out.println(Arrays.toString(getTnearestTrees(n,t,x,arr)));
    }
    static int[] getTnearestTrees(int n,int t,int x,Integer[] arr){
        int[] res = new int[t];
        Arrays.sort(arr,(x1,x2)->{
            if(Math.abs(x1-x)!=Math.abs(x2-x))
              return Integer.compare(Math.abs(x1-x),Math.abs(x2-x));
