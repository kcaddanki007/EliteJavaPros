...
import java.lang.*;
class day11p4{
        /*static int getRect(int[][] arr,int m, int n){
            int ans=0;
            for(int i=0;i<m;i++){
                for(int j=0;j<n;j++){
                    if(arr[i][j] == 1){
                        List<Integer> list = new ArrayList<>();
                        for(int tempi=i+1;tempi<m;tempi++)
                          if(arr[tempi][j]==1) list.add(tempi);
                        
                        while(!list.isEmpty()){
                            int otheri = list.remove(list.size()-1);
                            
                            for(int tempj=j+1;tempj<n;tempj++)
                                if(arr[otheri][tempj] == 1 && arr[i][tempj]==1) ans++;
                        }
                    }
                }
            }
		...
		//System.out.println(getRect(arr,m,n));
		}
}        