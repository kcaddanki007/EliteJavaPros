...
class Solution {
    public String networkToString(BinaryTreeNode t) {
        ...
        if(t==null || t.data==-1) return "";
        
        if((t.left == null || t.left.data==-1) && (t.right==null || t.right.data==-1)) return ""+t.data;
        
        StringBuilder sb = new StringBuilder();
        sb.append(t.data);
        sb.append('[').append(networkToString(t.left)).append(']');
        String temp = networkToString(t.right);
        if(temp.length()!=0) sb.append('[').append(temp).append(']');
