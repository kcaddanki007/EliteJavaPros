...
class Solution
{
    public static void main(String args[])
    {
        ...
        System.out.println(getTernaryResult(s));
    }
    public static String getTernaryResult(String s)
    {
        StringBuilder str = new StringBuilder(s);
        int x = str.lastIndexOf("?");
        while(x>=1)
        {
            String temp = str.toString().substring(x-1,x+4);
            str.delete(x-1,x+4);
            if(temp.charAt(0)=='Y')
                str.insert(x-1,temp.charAt(2));
            else
                str.insert(x-1,temp.charAt(4));
            x = str.lastIndexOf("?");
        }
        return str.toString();
    }
