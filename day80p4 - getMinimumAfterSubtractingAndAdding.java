...
class Solution{
    public static void main(String[] args){
        ...
        System.out.println(getMin(0,nums,0,0));
    }
    static int getMin(int idx,int[] nums,int s1,int s2){
        if(idx==nums.length) return Math.abs(s1-s2);
        return Math.min(getMin(idx+1,nums,s1+nums[idx],s2),getMin(idx+1,nums,s1,s2+nums[idx]));
    }
}