...
class Solution{
    public static void main(String[] args){
        ...
        int[] start = new int[strarr.length];
        for(int i=0;i<strarr.length;i++)
           start[i] = Integer.parseInt(strarr[i]);
           
        strarr = sc.next().split(",");
        int[] end = new int[strarr.length];
        for(int i=0;i<strarr.length;i++)
          end[i] = Integer.parseInt(strarr[i]);
        
        System.out.println(Arrays.toString(inAp(arr,start,end)));
    }
    static boolean[] inAp(int[] arr,int[] start,int[] end){
        boolean[] res = new boolean[start.length];
        for(int i=0;i<res.length;i++){
            List<Integer> al = new ArrayList<>();
            for(int j=start[i];j<=end[i];j++) al.add(arr[j]);
            Collections.sort(al);
            if(al.size()==2) res[i]=true;
            for(int k=2;k<al.size();k++){
                if(al.get(k)-al.get(k-1) != al.get(k-1)-al.get(k-2)){
                    res[i]=false;
                    break;
                }else{
                    res[i]=true;
